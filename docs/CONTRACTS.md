...
| Agent streaming (G61) | `POST /api/workspaces/{id}/agent/run-stream` — SSE twin of `agent/run`: frames `started` → `tool_step` (per executed tool) → terminal `finished` carrying the FULL sealed run record (gated runs stream exactly one `finished` frame with the `not_run` record) or terminal `error`. Non-resumable by design: recovery reloads `GET /api/workspaces/{id}/agent/runs?limit=` (newest sealed `agent_run` transcripts — the replay source). Stream route is in the G58 quota set; the runs listing is a pure read |
...
| Comparative agent (G63) | `POST /api/workspaces/{id}/agent/compare` (`{objective, comp_workspace_ids (1..3), max_steps_per_workspace?}`, LLM quota set) — one per-workspace grounded agent run each (harness-scoped; tool calls can never cross workspaces), unanimous consent required (any non-consenting workspace → `not_run` naming `blocking_workspace_id`; never silent exclusion), DETERMINISTIC merge into per-workspace markdown sections (withheld/failed runs render as explicit annotations, their content never merged), belt-and-braces union grounding, sealed as `agent_comparative_run` on the primary workspace |

...
| POST | `/api/model-ops/prompt-ab` | `{prompt_id, candidate_template}` → G81: judges the REGISTERED template (A) vs the candidate (B) over the committed golden set with the existing judge; `{status:"completed", a:{prompt_version,prompt_hash,faithful_rate,judged}, b:{prompt_hash_candidate,...}, winner}`; persisted per prompt in blob storage (history capped at 20, appended under a lock). 422 unknown prompt / blank candidate; `not_run` provenance in mock. Consent is two-track and fail-closed: a workspace-bound run inherits that workspace's consent, and this workspace-UNBOUND route (payload = the committed golden set only, never tenant data) requires the operator-level `GOLDEN_EVAL_LLM_ALLOWED=true` opt-in — otherwise `not_run` / `no_consent` with zero provider construction. **Promotion convention (the eval gate): a registered template may only change in a PR carrying a winning A/B report — the registry hash change is the tamper signal, the report is the evidence.** In the G58 LLM quota set |
...
| Share links (G44) | `POST /api/workspaces/{id}/share-links` (`{label?,scope?=read_only,expires_at?}` → `{share_link, token}`) — mints an opaque `dsh_…` token returned **once**; only its SHA-256 digest is stored. `GET /api/workspaces/{id}/share-links` → `ShareLink[]` (no secrets). `POST /api/share-links/{id}/revoke` → `ShareLink` (idempotent). `GET /api/shared/{token}` → `SharedWorkspaceSnapshot` — **public, session-less** (the token is the authorization): a read-only, **non-confidential** snapshot (workspace identity, target public-company identity, risk findings, counts) that deliberately excludes financial line items, valuation, QoE adjustments, memo bodies, and data-room content. A revoked/expired token → **410**; unknown/malformed → **404**. Management endpoints are workspace-/org-scoped (tenant-guarded; viewers 403). G76: every **successful** public read appends a best-effort, append-only view event (coarse context only: truncated `user_agent` + transport `client_host`, mirroring session storage — no fingerprinting; a failed insert never breaks the read; invalid/revoked tokens record nothing); `GET /api/share-links/{id}/analytics` → `ShareLinkAnalyticsOut` `{share_link, view_count, first_viewed_at, last_viewed_at, recent:[{viewed_at,user_agent}]}` (recent capped 20 newest-first; `share_link` carries `revoked_at` so views + one-click revoke surface together; org-scoped — cross-tenant 404; the public route never exposes analytics). The snapshot gains a **server-composed** `watermark` string ("Shared read-only · org · link digest-prefix · created date") rendered by `/shared/{token}` as a persistent visible overlay — a provenance deterrent, not DRM |
...
| Diligence agent (G57) | `POST /api/workspaces/{id}/agent/run` (`{objective, max_steps?: 8 (1..16), client_request_id?}` → `AgentRunOut`) — a budget-capped tool-use loop over a curated allowlist of GOVERNED, read-only/pure-compute workspace tools (overview, filing search, cited Q&A, risks, evidence, saved cases, in-memory underwriting scenarios); the harness scopes every call (the model never chooses the workspace) and the agent cannot write to any governed record. Consent-gated like every LLM path (`not_run` with reason `mock`/`no_consent`/`no_api_key`; zero provider calls). The final answer passes a fail-closed grounding gate — any quantity token or EV-### ref that never appeared in a tool result's **evidence projection** (or the objective) → `status="rejected_ungrounded"`, answer withheld, violations listed. The gate's source is each result's argument-echo-free projection (every tool declares its echo fields in the registry; e.g. `get_evidence.unresolved` is excluded), capped at the size the model saw, and `get_evidence` refs are shape-validated against the auditor's own `EV-\d{3,}` pattern — the agent cannot launder a fabricated figure or ref through a tool round-trip. `client_request_id` (`[A-Za-z0-9_-]{8,64}`, optional) makes the POST idempotent across BOTH run routes: in-flight duplicate → 409 `duplicate_in_flight`; already-sealed duplicate → the sealed record replayed (no second run/spend/seal); reused id with a different objective → 409 `request_id_reuse_mismatch`. Every executed run seals an append-only `agent_run` ArtifactVersion with the full step transcript (stamped with `client_request_id` when given), and deal-linked workspaces emit an `agent.run_completed` audit event. Statuses: `completed`\|`rejected_ungrounded`\|`budget_exhausted`\|`error`\|`not_run`. In the G58 LLM quota set |
| Agent streaming (G61) | `POST /api/workspaces/{id}/agent/run-stream` — SSE twin of `agent/run` (same body incl. `client_request_id?`): frames `started` → `tool_step` (per executed tool) → terminal `finished` carrying the FULL sealed run record (gated runs stream exactly one `finished` frame with the `not_run` record; an idempotent replay streams exactly one `finished` frame with the previously sealed record) or terminal `error`. Non-resumable by design: recovery reloads `GET /api/workspaces/{id}/agent/runs?limit=` (newest sealed `agent_run` transcripts — the replay source; clients match THEIR run by `client_request_id`, never by position). Stream route is in the G58 quota set; the runs listing is a pure read |
| Agent memo drafts (G59) | `POST /api/workspaces/{id}/agent-memo/draft` (LLM quota set), `GET /api/workspaces/{id}/agent-memo`, `POST /api/workspaces/{id}/agent-memo/{draft_id}/sections/decide` (`{section, decision: accept\|reject}`, HUMAN actor required → 401 without). The agent drafts fixed sections each through its own grounding-gated run (`drafted`\|`withheld`\|`error`); decisions mint append-only draft supersessions; `assembled_markdown` is built ONLY from accepted sections once all drafted sections are decided |
| Comparative agent (G63) | `POST /api/workspaces/{id}/agent/compare` (`{objective (≤1738 — the G57 cap minus the computed per-workspace framing overhead; schema and service caps are lockstep-tested), comp_workspace_ids (1..3), max_steps_per_workspace?}`, LLM quota set) — EVERY involved workspace (primary + body-addressed comps) is resolved through the caller's tenant scope, so a cross-tenant comp returns the same 404 as a missing one (no existence oracle, no consent-config leak). One per-workspace grounded agent run each (harness-scoped; tool calls can never cross workspaces), unanimous consent required (any non-consenting workspace → `not_run` naming `blocking_workspace_id`; never silent exclusion), DETERMINISTIC merge into per-workspace markdown sections (withheld/failed runs render as explicit annotations, their content never merged), belt-and-braces union grounding over objective + grounded answers + harness scaffold (tool results deliberately excluded), sealed as `agent_comparative_run` on the primary workspace |
| Agent proposals (G60) | Agent tools `propose_qoe_adjustment` / `propose_claim` mint into the existing four-eyes queues as proposer `agent:diligence` (claims verified with the G53 discipline — verbatim quote + value-in-quote or nothing minted; `extraction_version="agent-proposed-v1"`). Proposals surface in the review inbox for humans; the proposer≠decider rule plus the human-reviewer requirements make automation approval impossible while human approval is unchanged |
| Peer benchmarking (G64) | `GET /api/workspaces/{id}/peer-benchmark` → percentile ranks (midrank convention) for growth/margin vs the SEC frames-API reporting universe for the anchor year — `peer_scope` states honestly that frames carry no SIC so ranks are vs ALL US filers reporting the concept; coverage counts per metric with a 20-entity floor (below → percentile null + "insufficient peer coverage", status `partial`); growth requires the entity in BOTH frame years, margins in both concept frames; outages degrade per-metric or to `unavailable` with `source_error`, never false-clean |
| Dilution (G66) | `GET /api/workspaces/{id}/dilution` → per-fiscal-year shares outstanding, SBC, repurchases, and net dilution % from XBRL (CY-frame keyed — `fy` is never a period key), computed only across CONSECUTIVE tagged years (never across gaps); missing concept-years stay null with per-field notes; per-field-year citations bind concept/end/accession/form |
| Underwriting depth (G69–G73) | Five deterministic POSTs under `/api/workspaces/{id}/underwriting/`: `sensitivity-tornado` (per-driver ± impacts ranked by max abs delta; multiples shift relatively, `*_shift` drivers absolutely; inevaluable extremes are named rows, never dropped), `dividend-recap-solve` (max distribution at a period under leverage/coverage/FCCR/liquidity constraints via bisection; names the BINDING constraint, disclosing in `note` when several cross within the solver tolerance — or when an iteration-exhausted bracket makes the answer a lower bound; `infeasible` names the violated constraint at zero; `unbounded` honest), `facility-sizing` (peak intra-year revolver need from G25 seasonality scaled by annual NWC; headroom = commitment − existing draw − peak draw; no observations → unavailable), `fund-monte-carlo` (N deals or a saved fund's G29-sized deals; SHARED macro factor draws (rate/growth/multiple with per-deal loadings) + idiosyncratic draws, one seed, byte-identical reruns; fund IRR/MoIC bands, per-deal marginals, and `correlation_effect` = the same seed and draws with every loading zeroed — a no-factor-variance baseline, not deals drawing factors independently — so the band-widening is shown, not asserted), `annual-value-creation` (the G22 bridge per hold year with Decimal-exact per-year and telescoping reconciliation; multiple change allocated to the final year — documented). Errors → 422 |
| Sum-of-the-parts (G65) | `POST /api/workspaces/{id}/sotp` (`{multiples:{segment: x}, default_multiple?, residual_multiple?}`) → per-segment implied EV from the stored G12 segment extract with an EXPLICIT unallocated residual (valued only when `residual_multiple` is supplied — never force-balanced; G12 `partial` reconciliation propagates); consolidated-only workspaces report `unavailable` |
| Litigation (G67) | `GET /api/workspaces/{id}/litigation` → Item 3 (Legal Proceedings) extraction from the latest 10-K (bounded by Item 4 with an Item 5 fallback, chunked with citations) + the 8-K legal-events timeline (explicitly-legal item codes only — the 8.01 limitation is stated); unfetchable text → `unavailable`, never clean. Also a risk-flag source in the analysis pipeline (degraded-source discipline) |
| Macro MC presets (G68) | `GET /api/workspaces/{id}/macro-mc-presets` → transparent, versioned (v1) mappings from FRED series (DGS10 → base_rate_shift, BAA10Y spread → exit_multiple, GDP → revenue_growth_shift) to ready-to-edit `MonteCarloRequest.distributions` entries with documented coefficients + per-series provenance; unavailable/thin-history series are omitted with notes, never fabricated |
| Redactions (G75) | `POST /api/deals/{id}/intelligence/documents/{document_id}/redactions` (`{spans:[{chunk_id,start,end,reason?}], note?}` → 201 `RedactionProposalOut`) — span-level redaction proposed against the LATEST document version (superseded → 409 stale); spans validated against real immutable chunks (unknown chunk / out-of-range / overlapping → 422); proposer must be a HUMAN session (trusted-service 403). `POST /api/intelligence/redactions/{proposal_id}/decide` (`{decision: approve\|reject, note?}` → `{proposal, redacted_document\|null}`) — approver must be a distinct human; decisions final (re-decide 409); approve mints version N+1 via the standard supersession path with `[REDACTED]` spliced over each span (metadata `{redaction_of, proposal_id, spans_count}`); reject mints nothing. `GET /api/deals/{id}/intelligence/redactions?status=&limit=` → list. Non-privileged surfaces (QA, cross-corpus, search, shares, agent tools) serve the redacted latest by latest-version-wins — zero special-casing; unredacted originals stay byte-identical privileged history. Binary docs: v1 redacts the extracted-text surface (documented boundary) |
| Export attestation (G74) | When `EXPORT_SIGNING_KEY` (base64url 32-byte Ed25519 seed) is configured, bundle + IC-export manifests gain `attestation = {status:"signed", signed_payload, signature_b64, public_key_b64, algorithm:"Ed25519"}` — the signature covers the exact canonical bytes whose SHA-256 is the manifest/bundle hash (never self-referential); unset → `{status:"unsigned", note}` and hash discipline is unchanged. Verification endpoints add an `attestation_signature` check when signed (a fully re-hashed forgery passes every hash check but fails the signature alone); the bundle ships `VERIFY.md` with the embedded public key for fully offline verification |
| Digests (G77) | `GET /api/organizations/{id}/notifications/digest?window=daily\|weekly` → `{organization_id, user_id, window, since, until, total, by_event_type:[{event_type,count,latest_title}], directed_count, inbox:{total, counts_by_plane, oldest_age_hours, sla:{total_breaches, breaches_by_plane}}}` — computed on read (no schema change, no read_at mutation); directed-notification privacy carries over (another member's mentions never appear) |
| Inbox SLAs (G78) | `GET /api/organizations/{id}/my-reviews/aging?actor_id=&<plane>_sla_hours=` → per-plane `{count, oldest_age_hours, sla_hours, breaches:[{id,title,age_hours,sla_hours}]}` + totals; defaults qoe/claim 72h, diligence 120h, ic_comment 48h; four-eyes exclusions hold (an item you proposed is not your breach) |

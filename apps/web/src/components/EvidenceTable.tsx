...
function confidenceFill(confidence: number): string {
  ...
  if (confidence >= 0.5) return "bg-severity-medium";
  return "bg-negative";
}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1.5 w-16 overflow-hidden rounded-full bg-sunken">
        <div
          className={`h-full rounded-full ${confidenceFill(confidence)}`}
          style={{ width: `${pct}%` }}
        />
      </div>
  ...
  return (
    <div className="space-y-4" data-workspace={workspaceId}>
      <Callout tone="info" title="Evidence & audit trail">
        ...
        by claim type — <span className="font-medium text-body">facts</span>,{" "}
        <span className="font-medium text-body">calculations</span>,{" "}
        <span className="font-medium text-body">inferences</span>, and{" "}
        <span className="font-medium text-body">assumptions</span> — with its source (an XBRL concept
        or a 10-K passage on sec.gov) and a confidence read. Outputs are not investment advice.
      </Callout>
          ...
          <TBody>
            {evidence.map((e) => (
              <tr
                key={e.id}
                id={e.ref}
                className="scroll-mt-24 border-b border-line-faint last:border-0 target:bg-accent-soft hover:bg-panel2"
              >
                <TD>
                  <span className="font-mono text-xs font-medium text-body">{e.ref}</span>
                </TD>
                <TD>
                  <ClaimBadge type={e.claim_type} />
                </TD>
                <TD className="text-body">{e.claim}</TD>
                <TD>
                  {e.source_url ? (
                    <a
                      href={e.source_url}
                    ...
                    <span className="font-medium text-ink">{e.source_name}</span>
                  )}
                ...
                <TD>
                  <span className="text-xs text-muted">{e.agent_name}</span>
                </TD>
              </tr>
            ))}
          </TBody>
